...
var PictureShow = React.createClass({
  ...
  goToSlide: function goToSlide(slideIdx, direction, event) {
  ...
  _handleResize: (0, _lodashFunctionThrottle2['default'])(function () {
  ...
  _handleSlideClick: function _handleSlideClick(event) {
  ...
  _handleSwipe: function _handleSwipe(ev, x, y, isFlick) {
    this.setState({
      swiping: false
    });
  ...
  _handleSwiping: function _handleSwiping() {
    // TODO: track finger
    this.setState({
      swiping: true
    });
  ...
  _getLeftDistance: function _getLeftDistance(startIdx, endIdx) {
    var d = startIdx - endIdx;
    return d < 0 ? React.Children.count(this.props.children) + d : d;
  },

  _getRightDistance: function _getRightDistance(startIdx, endIdx) {
    return this._getLeftDistance(endIdx, startIdx);
  },

  _shouldLoad: function _shouldLoad(idx) {
    if (this.preloaded.indexOf(idx) > -1) {
      return true;
    } else if (this._getLeftDistance(this.state.slideIdx, idx) <= this.props.slideBuffer || this._getRightDistance(this.state.slideIdx, idx) <= this.props.slideBuffer) {
  ...
  _getPanelStyle: function _getPanelStyle(idx, key) {
  ...
  render: function render() {
    ...
    var mainClass = ['picture-show', !this.props.ratio ? 'stretch' : undefined, this.props.className].join(' ');
    ...
    React.Children.forEach(this.props.children, (function (slide, idx) {
      ...
      slides.push(React.createElement(
        'div',
        { className: 'ps-slide-wrap', key: idx, style: slideStyle },
        slideContent
      ));
    }).bind(this));
    ...
    return React.createElement(
      ...
      React.createElement(
        'div',
        { className: 'ps-wrap', style: wrapStyle, ref: 'wrap' },
        ['A', 'B', 'C'].map((function (key) {
          var panelStyle = this._getPanelStyle(this.state.panels.indexOf(key), key);
          return React.createElement(
            'div',
            { className: 'ps-slides', key: key, style: panelStyle, onMouseDown: this._handleSlideClick },
            slides
          );
        }).bind(this))
      )
